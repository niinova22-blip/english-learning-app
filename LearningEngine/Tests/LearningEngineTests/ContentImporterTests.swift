...
final class ContentImporterTests: XCTestCase {
    ...
    func test_importPackage_realYDSPackage_importsEveryUnitItemAndQuestion() throws {
        ...
        XCTAssertEqual(Set(allItems.map(\.id)).count, 190, "duplicate item ids found")
        ...
        XCTAssertEqual(allQuestions.count, 583)
        XCTAssertEqual(Set(allQuestions.map(\.id)).count, 583, "duplicate question ids found")
