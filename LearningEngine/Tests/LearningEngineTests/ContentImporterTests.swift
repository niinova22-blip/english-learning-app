...
final class ContentImporterTests: XCTestCase {
    ...
    func test_importPackage_realYDSPackage_importsEveryUnitItemAndQuestion() throws {
        ...
        XCTAssertEqual(Set(allItems.map(\.id)).count, 200, "duplicate item ids found")
        XCTAssertTrue(allItems.allSatisfy { $0.content != nil })
        XCTAssertEqual(allItems.filter { $0.type == .vocabulary }.count, 120)
        XCTAssertEqual(allItems.filter { $0.type == .grammarPoint }.count, 42)
        XCTAssertEqual(allItems.filter { $0.type == .practiceSet }.count, 38)

        let allQuestions = allLessons.flatMap(\.questions)
        XCTAssertEqual(allQuestions.count, 643)
        XCTAssertEqual(Set(allQuestions.map(\.id)).count, 643, "duplicate question ids found")
