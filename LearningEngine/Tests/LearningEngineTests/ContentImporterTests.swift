...
final class ContentImporterTests: XCTestCase {
    ...
    func test_importPackage_realYDSPackage_importsEveryUnitItemAndQuestion() throws {
        ...
        XCTAssertEqual(Set(allItems.map(\.id)).count, 157, "duplicate item ids found")
        ...
        XCTAssertEqual(allQuestions.count, 335)
        XCTAssertEqual(Set(allQuestions.map(\.id)).count, 335, "duplicate question ids found")
