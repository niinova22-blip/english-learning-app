...
final class ContentImporterTests: XCTestCase {
    ...
    func test_importPackage_realYDSPackage_importsEveryUnitItemAndQuestion() throws {
        ...
        XCTAssertEqual(Set(allItems.map(\.id)).count, 165, "duplicate item ids found")
        ...
        XCTAssertEqual(allItems.filter { $0.type == .practiceSet }.count, 13)

        let allQuestions = allLessons.flatMap(\.questions)
        XCTAssertEqual(allQuestions.count, 375)
        XCTAssertEqual(Set(allQuestions.map(\.id)).count, 375, "duplicate question ids found")
