...
final class ContentImporterTests: XCTestCase {
    ...
    func test_importPackage_realYDSPackage_importsEveryUnitItemAndQuestion() throws {
        ...
        XCTAssertEqual(Set(allItems.map(\.id)).count, 181, "duplicate item ids found")
        ...
        XCTAssertEqual(allItems.filter { $0.type == .practiceSet }.count, 29)

        let allQuestions = allLessons.flatMap(\.questions)
        XCTAssertEqual(allQuestions.count, 511)
        XCTAssertEqual(Set(allQuestions.map(\.id)).count, 511, "duplicate question ids found")
